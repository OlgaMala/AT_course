...
class MinLenError(Exception):
    pass


def treatment_sum(our_tuple):
    '''
    Возвращает сумму двух элементов из переданного кортежа
    Использует класс MinLenError для вызова исключения, если в кортеже меньше 2 элементов
    :param our_tuple:
    :return: result
    '''
    try:
        if len(our_tuple) == 2:
            ...
            return result
        if len(our_tuple) < 2:
            raise MinLenError('Недостаточно данных')
        if len(our_tuple) > 2:
