...
def repeats(our_str):
    """
    Изменяет принимаемую на вход строку, используя цикл и добавление значения сколько раз уже встречался в строке символ.
    Записывает в переменную новую строку через '_' + количество вхождений.

    :param our_str:принимает на вход строку.
    :return:возвращает символы и количество вхождений в строку в формате l_1e_1t_1t_2e_2_r_1
    """
    new_str = ''
    for k in our_str[0:len(our_str)]:
        v = 1
        if k in new_str:
            v += new_str.count(k)
        new_str += '_'.join([str(k), str(v)])
