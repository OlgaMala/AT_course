...
def everything_for_your_cat(cats_data):
    """
    Создаем словарь, используя срезы, где ключ - имя и фамилия, а значение - кличка, возраст.
    Добавляем кличку и возраст через ","
    Проходим циклом по парам словаря, формируя кортежи из ключа и значений через ";",
    объединяем в строку, а строки возвращаем с новой строки в переменной our_str.

    :param cats_data:
    :return: our_str строки с новой строки с ключом и значениями по этому ключу
    """
    result = {}
    for item in cats_data:
        cat = item[0] + ', ' + str(item[1])
        result.setdefault(item[2:], []).append(cat)

    our_str = ''
    for k, v in result.items():
        our_str1 = ' '.join(k) + ':', '; '.join(v)
        our_str2 = ' '.join(our_str1)
        our_str += our_str2 + '\n'
    return our_str
